...
/// 密码找回：给用户发送重置链接邮件。
pub async fn send_password_reset_email(cfg: &Config, pool: &SqlitePool, email_addr: &str, token: &str) {
    db::log_event(pool, "pwd_reset_sent", Some(email_addr), "").await.ok();

    let Some(smtp) = &cfg.smtp else {
        tracing::warn!("未配置 SMTP，无法发送密码重置邮件：{email_addr}");
        return;
    };
    let link = format!("{}/reset-password?token={}", cfg.public_base_url.trim_end_matches('/'), token);
